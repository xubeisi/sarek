...
workflow ANNOTATE {
    ...
    if (tools.split(',').contains('merge') || tools.split(',').contains('snpeff')) {
        ...
        ch_tab_ann   = ch_tab_ann.mix(ANNOTATION_SNPEFF.out.summary_html)
        ch_json_ann  = ch_json_ann.mix(ANNOTATION_SNPEFF.out.genes_txt)
    ...
    if (tools.split(',').contains('vep')) {
        ...
        ch_tab_ann   = ch_tab_ann.mix(ANNOTATION_ENSEMBLVEP.out.tab)
        ch_json_ann  = ch_json_ann.mix(ANNOTATION_ENSEMBLVEP.out.json)
